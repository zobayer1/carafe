...
// The names go on the wire in an Allow header, where they are case-sensitive tokens rather than labels: a lowercase one
// is a different method, and a misspelled one is no method at all.
...
// Only a bug reaches this, since every Method comes from the parser. It answers empty rather than reading off the end
// of a table, which keeps the failure to one missing name instead of a malformed header.
TEST(MethodName, IsEmptyForAValueOutsideTheEnum) {
    // Well defined for a scoped enum, whose value range is its underlying type's; the check is conservative, and
    // reaching the fallback is the point here.
    // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
    EXPECT_TRUE(method_name(static_cast<Method>(99)).empty());
}

// The name comes from the pattern the route was registered with, so a handler asks for what it wrote rather than for
...
// A pattern binding one name twice is odd rather than wrong, and add() has no channel to refuse it on. The first reads
...
// Why get() answers an optional rather than an empty string: the router will not capture an empty segment, but Params
// is an open struct that cannot promise that, so empty has to stay a value and not a way of saying "absent".
...
// Unlike a header name, which arrives in whatever case the client chose, a parameter name is written twice by the same
// person, in the pattern and in the handler. Folding case would only hide a typo.
